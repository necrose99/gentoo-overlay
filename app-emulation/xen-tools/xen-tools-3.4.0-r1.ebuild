...
#	use vtpm && cp "${DISTDIR}"/${TPMEMUFILE}  tools/vtpm
	...
	# fix stupid .config issue
	epatch "${FILESDIR}/config-mk.patch"

	#Fix the stupid use of LD to check linker shit
	#when LD doesn't understand half of what comes
	#out of curl-config --libs
	epatch "${FILESDIR}/check-change.patch"
}
